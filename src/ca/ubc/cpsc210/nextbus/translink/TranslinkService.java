...
public class TranslinkService extends AbstractTranslinkService {
	 ...
	 */
	private final static String APIKEY = "EOZoJv2XmAhKPSeCg0i5";  // Replace with your own Translink API key
    ...
    public void addWaitTimeEstimatesToStop(BusStop stop) throws TranslinkException {
		// get stop number from stop
		int stopNum = stop.getStopNum();
		stop.clearWaitTimes(); // clear wait time before adding
		// create uri to query Translink for stop estimates (as per Translink API website)
		StringBuilder uriBuilder = new StringBuilder(
				"//api.translink.ca/RTTIAPI/V1/");
    ...
    public void addBusLocationsForStop(BusStop stop)
		...
		StringBuilder uriBuilder = new StringBuilder(
				"//api.translink.ca/RTTIAPI/V1/");
		// stop.getStopNum gets the stop number from stop
		uriBuilder.append("buses?");
		uriBuilder.append("apikey=" + APIKEY);
		uriBuilder.append("&stopNo=" + stop.getStopNum());
		stop.clearBuses(); // clear buses for stop before adding

		// try send query to the uri just built and get response from Translink as responseBody
		try {
			String responseBody = makeJSONQuery(uriBuilder);
